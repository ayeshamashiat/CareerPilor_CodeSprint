...
const MOCK_TAILORED_CVS = [
  {
    id: 1,
    jobTitle: 'Backend Engineer',
    company: 'bKash',
    createdAt: 'Jun 2, 2025',
    changes: 'Reordered sections to lead with backend experience. Rewrote 4 bullet points to highlight API design and Redis usage.',
  },
  {
    id: 2,
    jobTitle: 'SWE Intern',
    ...
    changes: 'Emphasised Python skills and data pipeline work. Added relevant keywords from JD.',
  },
  {
    id: 3,
    ...
    changes: 'Moved analytics projects to the top. Highlighted SQL and visualisation experience.',
  },
]
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="bg-gray-800/50 border border-gray-800 rounded-lg px-3 py-2">
        <span className="text-sm text-white">{value || <span className="text-gray-600 italic">Not set</span>}</span>
      </div>
    </div>
  )
}
...
function TailoredCVCard({ cv, onDownload, onDelete }) {
      ...
      <div className="bg-gray-950 border border-gray-800 rounded-xl overflow-hidden">
        <div className="flex items-start justify-between px-4 py-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-white">{cv.jobTitle}</p>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="flex items-center gap-1 text-xs text-gray-500">
                  <Briefcase size={10} /> {cv.company}
                </span>
                <span className="text-gray-700">·</span>
                <span className="flex items-center gap-1 text-xs text-gray-500">
                  <Calendar size={10} /> {cv.createdAt}
                </span>
              </div>
            </div>
          </div>
        ...
        {expanded && (
          <div className="px-4 pb-4 border-t border-gray-800 pt-3">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">{cv.changes}</p>
          </div>
        )}
      </div>
    </>
...
const handlePasswordSave = async () => {
  ...
  const handleTailoredDelete = async (id) => {
    try {
      // await axios.delete(`http://localhost:8000/api/tailored-cvs/${id}`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      <div className="px-6 py-5 max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
          ...
          <p className="text-xs text-gray-600 -mt-2">
            Your base CV used to generate all tailored versions.
          </p>
          ...
          {mainCVName ? (
            ...
            <div className="flex items-center gap-3 bg-gray-950 border border-gray-800 rounded-xl px-4 py-4">
              ...
              <p className="text-sm text-gray-600">No CV uploaded yet. Upload one from the CV page.</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-3">
          ...
          <p className="text-xs text-gray-600 -mt-2">
            CVs auto-generated for specific job applications.
          </p>

          {tailoredCVs.length === 0 ? (
            <p className="text-sm text-gray-600 text-center py-6">
              No tailored CVs yet — head to <span className="text-violet-400">Tailor CV</span> to generate one.
            </p>
          ) : (
            <div className="space-y-2">
              {tailoredCVs.map((cv) => (
                <TailoredCVCard
                  key={cv.id}
                  cv={cv}
                  onDownload={handleTailoredDownload}
