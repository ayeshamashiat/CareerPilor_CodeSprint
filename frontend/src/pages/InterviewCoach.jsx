...
useEffect(() => {
  ...
  const timerColor = timeLeft !== null && timeLeft <= 30
    ? 'text-red-400' : timeLeft <= 60 ? 'text-yellow-400' : 'text-green-400'
          ...
          <div className="bg-gray-900 rounded-2xl p-6 space-y-5">
            ...
            <div>
              ...
              <textarea value={jobDescription} onChange={e => setJobDescription(e.target.value)}
                rows={6} className="w-full bg-gray-800 text-white rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-violet-500 resize-none"
                ...
                {[5, 7, 10].map(n => (
                  <button key={n} onClick={() => setNumQuestions(n)}
                    className={`flex-1 py-2 rounded-lg text-sm font-semibold transition ${
                      numQuestions === n ? 'bg-violet-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'
                    }`}
                  >{n} Questions</button>
                ))}
              </div>
            ...
            <div className="bg-gray-800 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-white font-medium">Timer per question</span>
                </div>
                <button onClick={() => setUseTimer(p => !p)}
                  className={`w-11 h-6 rounded-full transition-colors relative ${useTimer ? 'bg-violet-600' : 'bg-gray-600'}`}
                >
                  ...
                  {[60, 120, 180].map(d => (
                    <button key={d} onClick={() => setTimerDuration(d)}
                      className={`flex-1 py-1.5 rounded-lg text-xs font-semibold transition ${
                        timerDuration === d ? 'bg-violet-600 text-white' : 'bg-gray-700 text-gray-400 hover:text-white'
                      }`}
                    >{d / 60} min</button>
                  ))}
                </div>
                ...
                <div className="space-y-2">
                  {sessions.map(s => (
                    <div key={s.id} className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 flex items-center justify-between">
                      <div>
                        <p className="text-sm font-semibold text-white">{s.job_title || 'Untitled'}</p>
                        <p className="text-xs text-gray-500">{s.created_at} · {s.questions_count} questions</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className={`text-sm font-bold ${
                          s.overall_score >= 75 ? 'text-green-400' : s.overall_score >= 55 ? 'text-yellow-400' : 'text-red-400'
                        }`}>{s.overall_score}%</span>
                        <button onClick={() => handleExport(s.id)}
                          className="p-1.5 rounded-lg text-gray-500 hover:text-violet-400 hover:bg-gray-800 transition"
                          title="Export PDF"
                        >
                          <Download size={14} />
                        </button>
  ...
  if (phase === PHASES.DONE) {
    return (
      <div className="px-4 py-10">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-3xl font-bold text-white mb-6">Interview Complete</h1>
          ...
          <div className="space-y-4 mb-6">
            {allEvals.map((item, i) => (
              <div key={i} className="bg-gray-900 rounded-2xl p-5">
                ...
                <p className="text-sm text-white mb-1">Score: <span className={
                  item.evaluation.overall_score >= 75 ? 'text-green-400 font-bold' :
                  item.evaluation.overall_score >= 55 ? 'text-yellow-400 font-bold' : 'text-red-400 font-bold'
                }>{item.evaluation.overall_score}%</span></p>
                {item.evaluation.ideal_answer_hint && (
                  <p className="text-gray-400 text-xs italic border-l-2 border-violet-500 pl-2 mt-2">
                    💡 {item.evaluation.ideal_answer_hint}
                  </p>
                )}
              </div>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 bg-gray-800 hover:bg-gray-700 border border-gray-700 text-white text-sm font-semibold py-3 rounded-lg transition flex items-center justify-center gap-2"
            >
        ...
        {cvContext.length > 0 && (
          <>
            <button onClick={() => setShowCVContext(p => !p)}
              className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-violet-400 mb-2 transition"
            >
              ...
              <div className="space-y-2 mb-4">
                {allEvals.map((item, i) => (
                  <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3">
                    <p className="text-xs text-violet-400 font-semibold">Q{i + 1}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{item.question}</p>
                    <p className="text-xs text-white mt-1">{item.answer}</p>
          ...
          <div className="bg-gray-900 rounded-2xl p-6 space-y-4">
            ...
            {evaluation.strengths?.length > 0 && (
              <div>
                ...
                {evaluation.strengths.map((s, i) => <p key={i} className="text-gray-300 text-sm">• {s}</p>)}
              </div>
            )}
            ...
            {evaluation.improvements?.length > 0 && (
              <div>
                ...
                {evaluation.improvements.map((s, i) => <p key={i} className="text-gray-300 text-sm">• {s}</p>)}
              </div>
            )}
            ...
            <div className="flex gap-3">
              <button onClick={handleRetry}
                className="px-4 py-2.5 rounded-lg bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-300 text-sm font-medium transition flex items-center gap-2"
              >
